import { pgTable, text, timestamp, boolean, uuid, varchar, integer, index, uniqueIndex, pgEnum, date, jsonb } from "drizzle-orm/pg-core";
 ...
 */
export const customerStatusEnum = pgEnum("customer_status", ["active", "inactive", "vip"]);
...
export const customers = pgTable("customers", {
    ...
    source: varchar("source", { length: 50 }), // e.g., walk_in, referral, online, social_media
    ...
    preferences: jsonb("preferences").default({}), // e.g., preferred branch, communication preferences
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull().$onUpdateFn(() => new Date()),
}, (table) => {
    return {
        ...
        idxCustomersStatus: index("idx_customers_status").on(table.organizationId, table.status),
        idxCustomersSource: index("idx_customers_source").on(table.organizationId, table.source),
        idxCustomersCreated: index("idx_customers_created").on(table.organizationId, table.createdAt),
    }
});
